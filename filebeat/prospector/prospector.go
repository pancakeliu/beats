...
func (p *Prospector) Start() {
	...
	go func(hr *harvesterRegistry, p *Prospector) {
		var closeRemoveTimeout *time.Timer = nil
		if p.config.CloseRemoveTimeout > 0 {
			closeRemoveTimeout = time.NewTimer(p.config.CloseRemoveTimeout)
		}

		for {
			select {
			// Applies when timeout is reached
			case <-closeRemoveTimeout.C:
				logp.Info("Closing removed file describ, because close_removed_timeout was reached.")
				closeRemoveTimeout.Reset(p.config.CloseRemoveTimeout)
			}
			hr.StopRemove(p.prospectorer)
		}
	}(p.registry, p)
}
...
func (p *Prospector) updateEventState(event *input.Event) error {
...
func (p *Prospector) startHarvester(state file.State, offset int64) error {
	...
	// logp.Info("Begin startHarvester")
	...
	// logp.Info("begin createHarvester(state)")
	...
	// logp.Info("begin updateEventState(input.NewEvent)")

	// State is directly updated and not through channel to make state update synchronous
	// err = p.updateEventState(input.NewEvent(state))
	err = p.updateEventState(input.NewEvent(state))
	if err != nil {
		return err
	}

	// logp.Info("begin h.Serup()")
	...
	if err != nil {
		...
		// updateErr := p.updateEventState(input.NewEvent(state))
